...
const getSurvivors = (arr, count) => {
    let survivors = arr.filter((n) => n[1] === true).slice(0, count);
...
const mostRelated = (arr) => {
    ...
    let s1 = `At ${percent}% common DNA, Specimen`;
    let s2 = `${num} is most similar to the`;
    let s3 = ` comparison Specimen.\n[Specimen ${num}: ${strandA} => `;
    let s4 = `Comparison: ${strandB}]`;
